...
parser.add_argument(
    "--num-codebook-vectors", type=int, default=1024, help="Number of codebook vectors."
)
...
transformer.load_state_dict(
    torch.load(os.path.join("checkpoints", "transformer_last_ckpt.pt"))
)
...
for i in tqdm(range(n)):
    ...
    vutils.save_image(
        sampled_imgs,
        os.path.join("results", "transformer", f"transformer_{i}.jpg"),
        nrow=4,
    )
